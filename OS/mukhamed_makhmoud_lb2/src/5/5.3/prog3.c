 ...
 * Это позволяет отслеживать очередность выполнения процессов при различных
 * настройках приоритетов.
 *
 * КОМПИЛЯЦИЯ И ЗАПУСК:
 * $ gcc prog3.c -o prog3
 * $ ./prog3 <приоритет>
 ...
 *
 * РЕЗУЛЬТАТЫ И ВЫВОДЫ:
 * - Демонстрирует работу планировщика FIFO при запуске с другими процессами
 * - Позволяет наблюдать, как политика FIFO влияет на порядок выполнения
 * - При запуске с одинаковыми приоритетами показывает, что процессы
 *   выполняются последовательно (первый получивший проц. время - первый обслуженный)
 * - При запуске с разными приоритетами демонстрирует приоритетность
 *   выполнения (процесс с более высоким приоритетом выполняется первым)
 */
